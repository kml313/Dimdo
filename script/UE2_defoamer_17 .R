...
u_design<-provide_u_design(dataList,V,u_names,u_roles,u_transform,u_gradient,u_offset,u_low,u_high,newData = FALSE)
design<-fetchDesign(2,VsAndWs$VRES,VsAndWs$WRES,u_design$u_design,infosFromU$u_low,infosFromU$u_high,DLFsettings$x_low,DLFsettings$x_high,infosFromU$u_roles,VsAndWs$x_roles,test_eqpr,desType,desParameters)
print(design$flag)
if(design$flag=="ok"){

  print(u2uu(design$u_design,u_transform,u_gradient,u_offset)$uu_vec)
  print(design$u_design%*%VsAndWs$VRES)
  plot(design$u_design%*%VsAndWs$VRES[,'Fr'],(design$u_design%*%VsAndWs$VRES)[,'cT'])
  print(cbind(design$x_D_low,design$x_D_high))
  print(design$MD)
  plot(design$MD[,'Fr'],design$MD[,'cT'])
  print(design$Dfrml)
...
#################################################################
u_new<-provide_u_design(dataList,V,u_names,u_roles,u_transform,u_gradient,u_offset,u_low,u_high,newData = TRUE)
#u_new<-uu2u(dataList$uu_new,u_transform,u_gradient,u_offset)
scaleUp<-scaleUpProcess((u_new$u_design),dataList$z_new,fittedModel,infosFromU$u_roles,infosFromU$u_low,infosFromU$u_high,VsAndWs$VRES,V_resp,design$x_D_low,design$x_D_high,validate=TRUE,debug=0)#changed added as.matrix
